...
BOOST_AUTO_TEST_CASE( testExecuteZeroKick )
{
    ...
    // Set up test particle kick table with single entry. 
    ...
    const Eigen::Vector3d perturbedBodyStateAfterKick 
        = executeKick( perturbedBodyStateBeforeKick, testParticleKickTable.begin( ), 1.0 );
...
BOOST_AUTO_TEST_CASE( testExecuteCancellingKicks )
{
    ...
    testParticleKickTable.insert( 
      new TestParticleKick( 1, 1, 1.0, 0.1, 0.5, 1.9, 
                            preConjunctionState, 1.5, 1.8, postConjunctionState ) );
    testParticleKickTable.insert( 
      new TestParticleKick( 2, 1, 1.3, 0.1, 0.8, 1.9, 
                            postConjunctionState, 1.8, 1.8, preConjunctionState ) );
    // Set perturbed body state before kick.
    const Eigen::Vector3d perturbedBodyInitialState( 1.0, 0.5, 1.2 );

    // Execute kicks on perturbed body.
    TestParticleKickTable::iterator iteratorKickTable = testParticleKickTable.begin( );
...
BOOST_AUTO_TEST_CASE( testExecuteGeneralKick )
{
    ...
    // Execute kicks on perturbed body.
    const Eigen::Vector3d perturbedBodyStateAfterKick 
        = executeKick( perturbedBodyInitialState, testParticleKickTable.begin( ), massRatio );
